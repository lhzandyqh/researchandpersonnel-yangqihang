      ...
      <el-dialog :visible.sync="showdetails" title="课题详细信息" width="600px" >
        <div class="clearfix">
          ...
          <el-row  style="padding-top: 10px">
            <el-col >
              <div>
                <div v-for="(item, key) in detailTableData" :key="key" v-if="item.type === '基本信息'">
          ...
          <el-row  style="padding-top: 10px">
            <el-col >
              <div>
                <div v-for="(item, key) in detailTableData" :key="key" v-if="item.type === '项目信息'">
                  <span class="detailName">{{item.contentName}}：</span>
                  <span class="detailValue">{{item.contentValue}}</span>
                </div>
              </div>
          ...
          <el-row  style="padding-top: 10px">
            <el-col >
              <div>
                <div v-for="(item, key) in detailTableData" :key="key" v-if="item.type === '预算信息'">
                  <span class="detailName">{{item.contentName}}：</span>
                  <span class="detailValue">{{item.contentValue}}</span>
                </div>
              </div>
...
<!--        <el-tabs v-model="activeName">-->
        <el-form ref="form" label-width="100px" style="margin-top: 40px">
          <el-form-item label="项目名称:">
            <el-select v-model="editProjectName" style="width: 50% " placeholder="请选择" @change="chooseExpert(editProjectName)">
              <el-option v-for="(item,key) in nameData" :key="key" :label="item.projectName" :value="item.projectName"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="评审专家:">
            <span class="expert">&#8195;{{editAssessExpert}}</span>
          </el-form-item>
...
<!--        </el-tabs>-->
    ...
    <div>
      <el-dialog title="专家评审意见" :visible.sync="viewResultVisible" width="40%" :before-close="handleClose">
        <span class="resultTitle">&#8195;{{middleResult}}</span>
        <span slot="footer" class="dialog-footer">
...
<script>
import {getProjectInterim,insertProjectInterim,updateProjectInterim,getConclusionProjectNAndExpert, getDiscussionResult} from '@/api/tecProjectManagement'
export default {
  name: 'ProjectMiddle',
  data() {
    return {
  ...
  methods: {
    ...
    chooseExpert(expert) {
      ...
      for(let i = 0; i < this.nameData.length; i ++) {
        if(expert === this.nameData[i].projectName) {
          this.editAssessExpert = this.nameData[i].assessExpert
        }
      }
      console.log(this.editAssessExpert)
    ...
    viewResultDetails(row) {
      ...
      getDiscussionResult (prams).then(response =>{
        console.log('测试教师查看中期评审结果')
        console.log(response.data)
        this.middleResult = response.data.data
      })
    },
    handleClose(done) {
      this.$confirm('确认关闭？')
...
// 教师获取项目中期所有信息
    ...
    switchType(type) {
      if (type === 'add') {
        ...
        this.a = 0
        const prams = {
          tUsername : localStorage.getItem('loginName')
        }
        getConclusionProjectNAndExpert(prams).then(response =>{
          console.log('测试获取项目名称和项目评审专家接口')
          console.log(response.data)
